...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var txtHebrewDate: TextView
    private lateinit var txtHolidayWarning: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txtHebrewDate = findViewById(R.id.txtHebrewDate)
        txtHolidayWarning = findViewById(R.id.txtHolidayWarning)
    ...
    private fun updateHebrewInfo() {
        if (!dateChosen) return
        txtHebrewDate.text = "תאריך עברי: ${HebrewDateHelper.hebrewDateString(calendar)}"

        val holiday = HebrewDateHelper.holidayName(calendar)
        val shabbat = HebrewDateHelper.isShabbat(calendar)

        txtHolidayWarning.text = when {
            holiday != null && shabbat -> "⚠️ שים לב: שבת וגם חג ($holiday)"
            holiday != null -> "⚠️ שים לב: התאריך הזה הוא חג ($holiday)"
            shabbat -> "⚠️ שים לב: התאריך הזה הוא שבת"
            else -> ""
        }
    }
    ...
    private fun refreshAppointmentsList() {
        ...
        for (entry in sorted) {
            val parts = entry.split("|")
            if (parts.size == 3) {
                val hebrewDate = HebrewDateHelper.hebrewDateString(parseDateKey(parts[0]))
                sb.append("${parts[0]} ($hebrewDate)  ${parts[1]}  -  ${parts[2]}\n")
            }
        }
